...
🟢 Complete — built as `/kentucky` (`src/app/kentucky/page.tsx`) with four sections: `KentuckyHero.tsx`, `KentuckyLocalConnection.tsx`, `KentuckyCitiesServed.tsx` (all 13 cities from the Jul 15 expanded list, resolving the earlier open question), and `KentuckyServicesSummary.tsx` (reuses the shared services list, links to `/services`). Ends with the shared `ContactCta`, extended with a `phone` prop so it can show the KY number instead of the TN default. Built 2026-07-21.
  ...
  - **Revised again same day per Ashley's actual email notes** (Jul 21, 2026): the no-photo hero decision above was reversed — she asked for a photo back, specifically "a black and white headshot as pictured in my email profile." Turns out two of her seven headshots (`ashley-headshot-02.jpg` and `-04.jpg`) are real studio B&W/sepia versions, not color originals needing a filter — these had never been copied from the docs repo into the site's `public/images` (only the four color ones had been). `KentuckyHero.tsx` now uses `ashley-headshot-02.jpg` (real B&W, no CSS filter) in a restored two-column layout. `KentuckyLocalConnection.tsx` copy was replaced with Ashley's exact wording from that email, and now also displays the Carol Martin Gatton Academy seal (`cmg-seal.png`, from her diploma, added to `assets/icons` by Tiffany) as a badge beside the text.
...
Status:
🟢 Complete — built as `/ohio` (`src/app/ohio/page.tsx`), structurally matching `/kentucky`: `OhioHero.tsx`, `OhioCitiesServed.tsx` (all 10 cities from the population-ranked list), `OhioServicesSummary.tsx` (reuses the shared services list), and the shared `ContactCta` with the OH phone number via its `phone` prop. No separate "Why Choose Ashley" or FAQ sections — deliberate decision to keep OH structurally consistent with KY rather than building the extra sections this row originally called for. Built 2026-07-21.
  - **Revised same day per Ashley's actual email notes:** her Ohio notes said "include a photo on this page" (no B&W request, unlike Kentucky) and gave exact copy — "With professional and personal ties to the state of Ohio, Ashley is devoted to providing skilled document examination services to the business transacting public across the state." `OhioHero.tsx` was updated to that wording and now uses `ashley-headshot-03.jpg` (color) in a two-column layout, matching Kentucky's restored hero pattern. Note the "personal ties to Ohio" line **contradicts** the Regional Background doc's earlier note that no such connection existed for Ohio — see that section, updated 2026-07-21, for detail. No dedicated personal-connection section was built for Ohio (unlike Kentucky's), since there's no further material from Ashley to expand it into yet.
...
**Pre-handoff audit fix (2026-07-21):** the JSON-LD schema originally included `email: siteConfig.email` on all three blocks — Tiffany caught that this put the plain-text address back into every prerendered page, undoing the `ObfuscatedEmail` fix from earlier the same day (see Contact Information above). `email` was removed from all three schema blocks; `telephone` and `address` still give search engines contact info without exposing the address to scrapers. Re-verified via `npm run build` + grep: zero occurrences of the email address anywhere in `.next/server/app` after the fix.
